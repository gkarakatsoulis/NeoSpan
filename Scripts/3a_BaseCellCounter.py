...
def has_reads(bam_file):
    """
    Check if a BAM file has any reads.
    Returns True if the file has reads, False otherwise.
    """
    bam = pysam.AlignmentFile(bam_file, "rb")
    try:
        # Check if there is at least one read in the file
        for read in bam:
            return True
    finally:
        bam.close()
...
def normalize_chromosome_name(chrom, bam):
    """ Normalize chromosome names between BAM and reference formats. """
    bam_chroms = set(bam.references)  # Get chromosome names from BAM file

    if chrom in bam_chroms:
        return chrom  # No need to change

    # Try stripping "chr"
    if chrom.startswith("chr") and chrom[3:] in bam_chroms:
        return chrom[3:]

    # Try adding "chr"
    if "chr" + chrom in bam_chroms:
        return "chr" + chrom

    # Handle mitochondrial chromosome naming
    if chrom in ["MT", "chrMT"] and "MT" in bam_chroms:
        return "MT"
    if chrom in ["MT", "chrMT"] and "chrM" in bam_chroms:
        return "chrM"

    # If no match is found, raise an error
    raise ValueError(f"Error: Contig '{chrom}' not found in BAM file")
...
def run_interval(interval, BAM, FASTA, MIN_COV, MIN_CC,MIN_AF, MIN_AC, tmp_dir, BQ, MQ, MAX_DP):
	...
	# print(f"DEBUG: Checking BAM file for reads at {CHROM}:{START}-{END}")
	bam = pysam.AlignmentFile(BAM, "rb")
	CHROM = normalize_chromosome_name(CHROM, bam)
	count = bam.count(CHROM, START, END)
	# print(f"DEBUG: Found {count} reads in {CHROM}:{START}-{END}")
...
def initialize_parser():
	parser = argparse.ArgumentParser(description='Script to obtain a list of base and cell counts in BAM files')
	parser.add_argument('--bam_folder', type=str, default=1, help='Folder containing the BAM files to be analysed', required = True)
...
def main():
    ...
    # List all BAM files in the provided folder
    ...
    # Filter BAM files with reads
	valid_bam_files = []
	excluded_bam_files = []

	for bam_file in bam_files:
	    if has_reads(bam_file):
	        valid_bam_files.append(bam_file)
	    else:
	        excluded_bam_files.append(bam_file)
    
	if excluded_bam_files:
	    print("The following BAM files were excluded because they contain 0 reads:")
	    for excluded_file in excluded_bam_files:
	...
	for BAM in valid_bam_files:
	    ...
	    out_file = os.path.join(OUTF, f"{ID}_{os.path.basename(BAM)}.tsv")
