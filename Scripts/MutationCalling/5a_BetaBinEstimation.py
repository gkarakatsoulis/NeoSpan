...
def initialize_parser():
	parser = argparse.ArgumentParser(description='Script to estimate the Beta-binomial distribution parameters (alpha and beta) to be used for initial mutation calling.')
	parser.add_argument('--tsv_dir', type=str, help='Folder containing the tsv files to be used for the beta-binomial fitting (obtained with BaseCellCounter.py script)', required = True)   
...
def main():
	...
	# 3. Find the tsv files in a the directory
	tsv_files_list = [os.path.join(tsv_dir, f) for f in os.listdir(tsv_dir) if f.endswith('.tsv')]
    
	if not tsv_files_list:
	     print(f"Error: No .tsv files found in directory {tsv_dir}")
	     sys.exit(1)

	print ('\n- Number of tsv files to be used:')
	print(f'  > {len(tsv_files_list)}')
