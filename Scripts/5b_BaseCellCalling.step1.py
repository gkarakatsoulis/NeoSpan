		...
		for line in f:
			if line.startswith('##'):
				...
				if line.startswith('#CHROM') and counter == 0:
					...
					# Info header
					DICT = {'ALT_mut': "##INFO=<ID=ALT,Description=Alternative alleles found>",
						'FILTER':"##INFO=<ID=FILTER,Description=Filter status of the variant site>",
						'Cell_types':"##INFO=<ID=Cell_types,Description=Cell type/s with the variant>",
						'Up_context':"##INFO=<ID=Up_context,Description=Up-stream bases in reference (4 bases)>",
						'Down_context':"##INFO=<ID=Down_context,Description=Down-stream bases in reference (4 bases)>",
						'N_ALT':"##INFO=<ID=N_ALT,Description=Cell type/s with the variant>",
						'Dp':"##INFO=<ID=Dp,Description=Depth of coverage (reads) in the cell type supporting the variant>",
						'Nc':"##INFO=<ID=Nc,Description=Number of distinct cells found in the cell type with the mutation>",
						'Bc':"##INFO=<ID=Bc,Description=Number of reads (base count) supporting the variants in the cell type with the mutation>",
						'Cc':"##INFO=<ID=Cc,Description=Number of distinct cells supporting the variant in the cell type with the mutation>",
						'VAF':"##INFO=<ID=VAF,Description=Variant allele frequency of variant in the cell type with the mutation>",
						'CCF':"##INFO=<ID=CCF,Description=Cancer cell fraction (fraction of ditinct cells) supporting the alternative allele in the cell type with the mutation>",
						'BCp':"##INFO=<ID=BCp,Description=Beta-binomial p-value for the variant allele (considering read counts)>",
						'CCp':"##INFO=<ID=CCp,Description=Beta-binomial p-value for the variant allele (considering cell counts)>",
						'Cell_types_min_BC':"##INFO=<ID=Cell_types_min_BC,Description=Number of cell types with a minimum number of reads covering a site>",
						'Cell_types_min_CC':"##INFO=<ID=Cell_types_min_CC,Description=Number of cell types with a minimum number of distinct cells found in a specific site>",
						'Rest_BC':"##INFO=<ID=Rest_BC,Description=Base counts (reads) supporting other alternative alleles in this site. BC;DP;P-value (betabin)>",
						'Rest_CC':"##INFO=<ID=Rest_CC,Description=Cell counts supporting other alternative alleles in this site. CC;NC;P-value (betabin)>",
						'Fisher_p':"##INFO=<ID=Fisher_p,Description=Strand bias test. Fisher exact test p-value between forward and reverse reads in variant and reference allele>",
						'Cell_type_Filter':"##INFO=<ID=Cell_type_Filter,Description=Filter status of the variant site in each cell type>"
						}
					for info in DICT.keys():
						outfile.write(DICT[info]+'\n')	

					# Add calling columns
					INFO = ['ALT_mut', 'FILTER', 'Cell_types','Up_context', 'Down_context','N_ALT', 'Dp', 'Nc', 'Bc', 'Cc', 'VAF', 'CCF', 'BCp', 'CCp', 'Cell_types_min_BC', 'Cell_types_min_CC', "Rest_BC", "Rest_CC",'Fisher_p', 'Cell_type_Filter']
					...
					for cell_type_i in cell_types_idx.keys():
						...
						if INFO_i.startswith('NA'):
							...
							GT,DP,NC,CC,BC,BQ,BCf,BCr = INFO_i.split('|')
